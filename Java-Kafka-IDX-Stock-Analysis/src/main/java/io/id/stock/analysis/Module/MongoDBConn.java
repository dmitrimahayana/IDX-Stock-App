...
public class MongoDBConn {
    ...
    private static final Logger log = LoggerFactory.getLogger(MongoDBConn.class.getSimpleName());

    public MongoDBConn(String connectionString){
...
//                System.out.println("Existing collection "+collectionName+" documentID " + newDoc.getString("id") + " modified document count: " + updateResult.getModifiedCount());
    ...
    public void insertOrUpdateAvro(String databaseName, String collectionName, GenericRecord genericRecord){
        try {
            mongodb = mongoClient.getDatabase(databaseName);
            collection = mongodb.getCollection(collectionName);
            Document newDoc = Document.parse(genericRecord.toString()); //Convert to Doc format
