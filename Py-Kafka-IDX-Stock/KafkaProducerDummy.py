...
if __name__ == '__main__':
    ...
    while True:
        try:
            for row in jsonData:
                close = random.uniform(row['low'], row['high'])  # For testing purpose
                stock = Stock(row['id'], row['ticker'], row['date'], row['open'], row['high'], row['low'], close,
                              row['volume'])
                company = Company(row['ticker'], row['ticker'], row['name'], row['logo'],
                                  str(datetime.today().strftime('%Y-%m-%d %H:%M:%S')))
                print(f"id:{row['id']} high:{row['high']} open:{row['open']} low:{row['low']} close:{close} "
                      f"name:{row['name']} logo:{row['logo']}")
